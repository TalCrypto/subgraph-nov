...
export function handleETHStatked(event: ETHStakedEvent): void {
    ...
    if(!totalElPoint.lastUpdatedTimestamp.isZero()) {
        ...
        totalElPoint.totalElPoint = totalElPoint.totalElPoint.plus(periodInHours.times(totalElPoint.totalElBalance));
    }
    totalElPoint.totalElBalance = totalElPoint.totalElBalance.plus(event.params.amount);
